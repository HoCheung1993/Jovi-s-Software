...
CReportCtrl::CReportCtrl() : m_pWndEdit(NULL)
{
	...
	m_imagelist.Create(16, 16, ILC_COLOR32, 2, 1);    // 创建图像序列CImageList对象
	CBitmap bitmap;
...
void CReportCtrl::OnColumnclick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	LVCOLUMN vcolumn;
	CString strText;
	ZeroMemory(&vcolumn, sizeof(vcolumn));
	WCHAR buff[12];
	memset(buff, 0, 12);
	...
	/*if (COL == 0)
	{
		...
		vcolumn.pszText = buff;
		vcolumn.cchTextMax = 12;
		GetColumn(0, &vcolumn);
		strText = vcolumn.pszText;
		if (strText == L"[V] 名稱")
		{
			strText = L"[  ] 名稱";
			bCheckAll = FALSE;
		...
		vcolumn.pszText = strText.GetBuffer(0);
		vcolumn.cchTextMax = strText.GetLength();
		SetColumn(0, &vcolumn);
		for (int i = 0; i < N; i++)
		{
			SetCheck(i, bCheckAll);
		}
	}
	else
	{
...
int CReportCtrl::InsertServerIterm(LPCTSTR lpName, LPCTSTR lpIpAddr, LPCTSTR lpPort, LPCTSTR lpUser, LPCTSTR lpStatus)
{

	SetImageList(&m_imagelist);
	const int IDX = InsertItem(this->GetItemCount(), _T(""));
	SetItemText(IDX, 0, lpName);
	SetItemText(IDX, 1, lpIpAddr);
	SetItemText(IDX, 2, lpPort);
	SetItemText(IDX, 3, lpUser);
	SetItemText(IDX, 4, lpStatus);
	return IDX;
}
