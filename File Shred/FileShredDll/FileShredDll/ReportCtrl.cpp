...
int CReportCtrl::InsertFileIterm(LPCTSTR lpPathText)
{
	...
	hImagelist = (HIMAGELIST)SHGetFileInfo(lpPathText, 0, &sfi, sizeof(SHFILEINFO), SHGFI_SYSICONINDEX | SHGFI_ICON | SHGFI_DISPLAYNAME);
	imagelist.Attach(hImagelist);
	SetImageList(&imagelist);
	const int IDX = InsertItem(0, _T(""), sfi.iIcon);
	SetCheck(IDX, TRUE);
	SetItemText(IDX, 0, sfi.szDisplayName);
	SetItemText(IDX, 1, lpPathText);
	SetItemText(IDX, 2, _T("未處理"));
	//删除图像列表，解除占用
	imagelist.Detach();
